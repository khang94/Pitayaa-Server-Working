...
public final class StringUtil {
	...
	public static String formatText(String data) {
		// String str =
		// "aáàảãạâấầẩẫậăắằẳẵặeéèẻẽẹêếềểễệiíìỉĩịuúùủũụưứừửữựoóòỏõọôốồổỗộơớờởỡợyýỳỷỹỵ";
	...
	public static String convertPhone(String countryCode,String phone){
		String out= "" ;
		int lengthCodeCountry = countryCode.length();
		String codePhone = phone.substring(0, lengthCodeCountry);
		System.out.println("code phone = " + codePhone);
		...
		else if (countryCode.equals(codePhone))
		{
			phone = phone.replaceAll("[^0-9+]", "");
			out = "+" + phone;
		}
